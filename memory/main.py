...
def guards_to_calm_equip_drop_count(guard_battle_num:int) -> []:
    drop_count = [2,2,2]  # Altana and Natus will always drop one equipment each.
    test_array = rng_10_array()
    ptr = 3 # Ignore first two rolls, we don't care about item drops.
    ...
    if guard_battle_num <= 5:
        if guard_battle_num <= 4:
            ...
            if (test_array[ptr] & 0x7FFFFFFF) % 255 < chance:  # Rikku kills these first
                drop_count[i] += 1
        ptr += 3
        if guard_battle_num <= 4:
            if (test_array[ptr+i] & 0x7FFFFFFF) % 255 < chance:
    ...
    # Assume two maze larvae for Yuna.
    chance = 60
