...
def battle_arena_results():
    global baseValue
    if process.read_bytes(baseValue + 0x00D2C9F1, 1) == 2:
        return True
    return False
...
def battle_target_id():
    ...
    retVal = process.read_bytes(key, 1)
...
def get_items_count():
    ...
    itemCounts = []
    for x in range(60):
        itemCounts.append(process.read_bytes(baseValue + 0x00D30B5C + x, 1))
    return itemCounts


def get_item_count_slot(item_slot) -> int:
    global baseValue
    return process.read_bytes(baseValue + 0x00D30B5C + item_slot, 1)
...
def get_overdrive_battle(character):
    ...
    retVal = process.read_bytes(basePointerAddress + offset, 1)
...
def tidus_escaped_state():
    ...
    basePointerAddress = process.read(basePointer)
    offset = 0xDC8
    retVal = not process.read_bytes(basePointerAddress + offset, 1)
...
def get_enemy_current_hp():
    ...
    while enemyNum < 27:
        ...
        if enemyNum == 20:
            maxHP = [process.read_bytes(key1, 4)]
            current_hp = [process.read_bytes(key2, 4)]
        else:
            nextHP = process.read_bytes(key1, 4)
            if nextHP != 0:
                maxHP.append(nextHP)
...
def get_enemy_max_hp():
    ...
    while enemyNum < 25:
        ...
        if enemyNum == 20:
            ...
            if maxHP != 0:
                maxHP.append(process.read_bytes(key1, 4))
                current_hp.append(process.read_bytes(key2, 4))
        enemyNum += 1
...
def menu_open():
    ...
    menuOpen = process.read_bytes(key, 1)
    if menuOpen == 0:
        return False
    else:
        return True
...
def menu_number():
    global baseValue
    return process.read_bytes(baseValue + 0x85B2CC, 1)
...
def s_grid_node_selected():
    ...
    nodeNumber = process.read_bytes(key, 1)
    key = baseValue + 0x0012BEB7F
    nodeRegion = process.read_bytes(key, 1)
...
def map_cursor():
    ...
    ret = process.read_bytes(basePointerAddress + 272, 1)
    print(ret)
    return ret
...
def set_tidus_slvl(levels):
    ...
    key = baseValue + 0x00D32097
    sLvl = process.write_bytes(key, levels, 1)
    return sLvl
...
def party_format_cursor_1():
    ...
    retVal = process.read_bytes(coord, 1)
    return retVal
...
def get_actor_id(actor_num):
    ...
    return process.read_bytes(basePointerAddress + offsetX, 2)
...
def overdrive_state():
    ...
    for x in range(20):
        offset = (0x94 * x) + 0x39
        retVal[x] = process.read_bytes(basePointerAddress + offset, 1)
...
def get_blitz_aggro(player_index: int = 99):
    ...
    if player_index in [6, 7, 8, 9, 10]:
        if process.read_bytes(ptrKey + offset, 1) == 255:
            return False
        else:
            return True
    else:
...
def blitz_coords():
    ...
    xVal = process.read_bytes(key, 1)
    xVal = xVal * -1
    key = baseValue + 0x00D37690
    yVal = process.read_bytes(key, 1)
    return [xVal, yVal]
...
def get_equip_abilities(equip_num):
    ...
    retVal[0] = process.read_bytes(key, 2)
    key = basePointer + (0x16 * equip_num) + 0x10
    retVal[1] = process.read_bytes(key, 2)
    key = basePointer + (0x16 * equip_num) + 0x12
    retVal[2] = process.read_bytes(key, 2)
    key = basePointer + (0x16 * equip_num) + 0x14
    retVal[3] = process.read_bytes(key, 2)
    return retVal
...
def weapon_armor_cursor():
    global baseValue
    return process.read_bytes(baseValue + 0x0146A5E4, 1)
...
def customize_menu_array():
    ...
    for x in range(60):
        ...
        retArray.append(process.read_bytes(baseValue + offset, 2))
...
def read_val(address, bytes=1):
    ...
    retVal = process.read_bytes(key, bytes)
    return retVal
...
def arena_array():
    ...
    for i in range(104):
        ...
        retArray.append(process.read_bytes(key, 1))
    return retArray
