...
def nextMemChange():
    ...
    global ext
    timeNow = datetime.datetime.now()
    fileMemChange = "Logs/" + game + "MemChange_ " + str(timeNow.year) + str(timeNow.month) + str(timeNow.day) + "_" + str(timeNow.hour) + "_" + str(timeNow.minute) + "_" + str(timeNow.second) + ext
...
class memChangeMonitor:
    def __init__(self, baseOffsetRef, isPointerRef = False, ptrOffsetRef = 0x0, typeRef = '4byte', childReport:int = 0):
        ...
        if childReport != 0
            ...
            self.childHandle = memChangeMonitor(self.baseOffset, True, self.ptrOffset, self.typeRef, 0)
