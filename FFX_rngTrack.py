...
def decideSkipZanLuck() -> bool:
    ...
    if faceCrit:
        ...
        attackCount += 3
    if futureAttackHitMiss(character=7, enemy="seymour_flux", attackIndex=attackCount) == False:
        print("### Miss on Omnis")
        return False
...
def nextActionEscape(character: int = 0):
    index = 20 + character
    escapeRoll = FFX_memory.s32(
        FFX_memory.rngArrayFromIndex(index=index, arrayLen=1)[1]) & 255
    ...
    if testMode:
        ...
        if str(FFX_memory.rngSeed()) in records.keys():
            if gameVars.oblitzRNGCheck() in records[str(FFX_memory.rngSeed())].keys():
                ...
                records[str(FFX_memory.rngSeed())].update(newVal[FFX_memory.rngSeed()])
        else:
            records.update(newVal)
    print(newVal)
