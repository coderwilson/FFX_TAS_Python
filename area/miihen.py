        ...
        if memory.userControl():
            ...
            if checkpoint > 3 and checkpoint < 11:
                if gameVars.csr():
                    ...
                    if hunterCoords[1] < tidusCoords[1]:
                        ...
                        if memory.getCoords()[1] > 1356.5:  # Into position
                            if memory.getCoords()[0] < -44:
                                FFXC.set_movement(1, 0)
                                memory.waitFrames(30 * 0.06)
                                FFXC.set_neutral()
                                memory.waitFrames(30 * 0.09)
                            else:
                                checkpoint += 1
                                print("Close to the spot")
                            print(memory.getCoords())
                        elif memory.getCoords()[0] < -43.5:  # Into position
                        ...
                        if memory.getCoords()[0] > -43.5:  # Into position
                            checkpoint += 1
                        ...
                        if memory.getCoords()[1] > 1358.5:  # Into position
                            checkpoint = 10
                        ...
                        if memory.miihenGuyCoords()[1] < 1380:
                            ...
                            print("Mark 2")
                            try:
                                if memory.lucilleMiihenCoords()[1] > 1400 and memory.userControl():
                                    miihenSkip = True
                                else:
                                    memory.clickToControl3()
                            except Exception:
                        ...
                        checkpoint += 1
                        print("Checkpoint reached:", checkpoint)
            elif checkpoint == 11 and not memory.hunterSpear():
                targetPathing.setMovement(
                    [memory.miihenGuyCoords()[0], memory.miihenGuyCoords()[1]])
                xbox.tapB()

            # Map changes
            elif checkpoint < 15 and memory.getMap() == 120:
                checkpoint = 15
            # General pathing
            elif targetPathing.setMovement(targetPathing.miihen(checkpoint)):
                checkpoint += 1
            ...
            if memory.turnReady():
                if checkpoint < 4:  # Tutorial battle with Auron
                    ...
                    memory.fullPartyFormat('djose')
                    memory.closeMenu()
                elif checkpoint == 25 and not memory.battleActive():  # Shelinda dialog
                    FFXC.set_neutral()
                    xbox.tapB()
                else:
                    FFXC.set_neutral()
...
def arrival2(selfDestruct, battleCount, SDencounterID):
    ...
    checkpoint = 15
    while memory.getMap() != 171:
        if memory.userControl():

            # Map changes
            if checkpoint == 27:
                if memory.getCoords()[1] > 2810:
                    checkpoint += 1
                elif gameVars.csr():
                    checkpoint += 1
                else:
            ...
            if screen.BattleScreen():
                battleCount += 1
                if checkpoint == 27 and not memory.battleActive():  # Shelinda dialog
                    xbox.tapB()
                else:
                    print("Starting battle")
                    battle.MiihenRoad()
                ...
                xbox.tapB()
            elif memory.diagSkipPossible():  # Exclude during the Miihen skip.
                if checkpoint < 6 or checkpoint > 12:
                    xbox.tapB()

            # Map changes
            elif checkpoint < 13 and memory.getMap() == 120:
                checkpoint = 13
            elif checkpoint < 20 and memory.getMap() == 127:
                checkpoint = 20
            elif checkpoint < 31 and memory.getMap() == 58:
                checkpoint = 31
...
def midPoint():
    checkpoint = 0
    memory.fullPartyFormat('tidkimwak')
    while not memory.battleActive():
        if memory.userControl():
            pDownSlot = memory.getItemSlot(6)
            if memory.getMap() != 171:
                FFXC.set_movement(0, 1)
                memory.awaitEvent()
                FFXC.set_neutral()
            elif checkpoint == 2 and memory.getItemCountSlot(pDownSlot) >= 10:
                checkpoint = 4
            elif checkpoint == 3:
                checkpoint = 4
            elif checkpoint == 5:
...
def lowRoad(selfDestruct, battleCount, SDencounterID):
    ...
    while memory.getMap() != 79:
        if memory.userControl():
            ...
            # Map changes
            elif checkpoint < 17 and memory.getMap() == 116:
                checkpoint = 17
            elif checkpoint < 28 and memory.getMap() == 59:
                checkpoint = 28
...
def wrapUp():
    ...
    xbox.SkipDialog(12)
    FFXC.set_neutral()
    memory.clickToControl()  # Seymour scene
    FFXC.set_movement(0, 1)
