...
class EncounterSlider(tk.Frame):
    def __init__(
        ...
        **kwargs,
        ...
        self.scale = tk.Scale(
            self, orient="horizontal", label=None, from_=min, to=max, command=command
        )
...
class TkEncountersInputWidget(tk.Frame):
    ...
    def get_input(self) -> str:
        ...
        for encounter in self.encounters:
            ...
            for _ in range(encs):
                input_data.append(
                    f"encounter {encounter.type} " f"{encounter.name} {initiative}"
                )
        return "\n".join(input_data)
    ...
    def __init__(self, parent, seed: int, *args, **kwargs) -> None:
        ...
        for encounter in encounters:
            ...
            input_widget.add_slider(
                encounter.label, encounter.min, encounter.default, encounter.max
            )
...
class TkEncountersPlannerInputWidget(tk.Frame):
    def __init__(self, parent, *args, **kwargs) -> None:
        ...
        combobox = ttk.Combobox(
            self, values=options, state="readonly", textvariable=self.selected_zone
        )
    ...
    def add_slider(
        self, label: str, min: int = 0, default: int = 1, max: int = 100
    ) -> None:
    ...
    def get_input(self) -> str:
        ...
        if zones:
            for _ in range(int(self.shown_encounters.get())):
                input_data.append(
                    f'encounter multizone {"/".join(zones)} ' f"{initiative}"
                )
        return "\n".join(input_data)
