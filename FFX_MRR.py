    ...
    while FFX_memory.getMap() != 119:
        ...
        if FFX_memory.userControl():
            if checkpoint == 1:
                ...
                if status[0] == 0: #Yuna not enough levels
                    checkpoint = 44
                elif status[1] == 0: #Kimahri not enough levels
                    checkpoint = 44
                elif status[2] == 0: #Valefor did not recharge.
                    checkpoint = 44
                else:
                ...
                #print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                #print("Current gil value: ", FFX_memory.getGilvalue())
                #print("Last gil value: ", lastGilValue)
...
def mainPath_old(wakkaLateMenu):
    FFXC.set_value('AxisLx', 0)
    FFXC.set_value('AxisLy', 0)
    FFX_memory.awaitControl()
...
def guiAndAftermath():
    ...
    while FFX_memory.getMap() != 93:
        if FFX_memory.userControl():
            if checkpoint < 3 and FFX_memory.getMap() == 131:
                checkpoint = 3
            elif checkpoint == 6:
                FFXC.set_value('AxisLx', -1)
                FFXC.set_value('AxisLy', 0)
            ...
            if checkpoint == 2: #Move and mash at the same time
                FFX_Xbox.tapB()
        else:
            FFXC.set_value('AxisLy', 0)
            FFXC.set_value('AxisLx', 0)
