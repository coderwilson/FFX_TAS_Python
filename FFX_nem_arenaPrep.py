...
def autoLife():
    ...
    while FFX_memory.battleMenuCursor() != 22:
        if FFX_Screen.turnTidus() == False:
            print("Attempting Auto-life, but it's not Tidus' turn")
            FFX_Xbox.tapUp()
            FFX_Xbox.tapUp()
            return
                ...
                if FFX_memory.turnReady():
                    if battleNum in [431, 442]:
                        ...
                        if FFX_memory.getBattleNum() in [386] and not autoLifeUsed:
                            ...
                            FFX_Battle.useItemTidus(
                                FFX_memory.getUseItemsSlot(41))
                        ...
                        if battleNum in [377, 382]:
                            ...
                            FFX_Battle.useItem(
                                FFX_memory.getUseItemsSlot(41), rikkuFlee=True)
                        elif battleNum == 386:
                            # Armor bomber guys
                            FFX_Battle.useItem(
                                FFX_memory.getUseItemsSlot(41), rikkuFlee=True)
                            ...
                            FFX_Battle.useItem(
                                FFX_memory.getUseItemsSlot(63), rikkuFlee=True)
                        elif battleNum == 426:
                            # Master Tonberry
                            if not sleepPowder:
                                FFX_Battle.useItem(
                                    FFX_memory.getUseItemsSlot(37), rikkuFlee=True)
                            else:
                                if FFX_memory.getUseItemsSlot(41) < 100:
                                    FFX_Battle.useItemTidus(
                                        FFX_memory.getUseItemsSlot(41))
...
def transition():
    ...
    returnToAirship()
    FFX_memory.awaitControl()
    FFX_menu.addAbility(owner=0, equipment_type=0, ability_array=[
                        0x807A, 255, 255, 255], ability_index=0x8001, slotcount=2, navigateToEquipMenu=True, fullMenuClose=True)
...
def kilikaShop():
    ...
    FFX_menu.addAbility(owner=6, equipment_type=0, ability_array=[
                        0x800B, 0x8000, 255, 255], ability_index=0x8001, slotcount=4, navigateToEquipMenu=True, fullMenuClose=True)
    FFX_menu.addAbility(owner=0, equipment_type=1, ability_array=[
                        0x8072, 255, 255, 255], ability_index=0x8056, slotcount=4, navigateToEquipMenu=True, fullMenuClose=True)
...
def apToXP():  # Calm Lands purchases
    ...
    FFX_menu.addAbility(owner=0, equipment_type=0, ability_array=[
                        0x807A, 255, 255, 255], ability_index=0x8011, slotcount=2, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=False)
...
def autoPhoenix():  # Calm Lands items
    ...
    while FFX_memory.getItemCountSlot(FFX_memory.getItemSlot(7)) != 99:
        FFX_nem_arenaSelect.arenaMenuSelect(4)
        arenaNPC()
        print("Mega-Phoenix Downs:",
              FFX_memory.getItemCountSlot(FFX_memory.getItemSlot(7)))
    ...
    FFX_menu.addAbility(owner=4, equipment_type=1, ability_array=[
                        0x8072, 255, 255, 255], ability_index=0x800A, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=False)

    FFX_memory.waitFrames(30)
    initArray = FFX_memory.checkAbility(ability=0x8002)
    print("Initiative weapons:", initArray)
    if initArray[4]:
        FFX_menu.addAbility(owner=6, equipment_type=1, ability_array=[
                            0x8072, 255, 255, 255], ability_index=0x800A, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=False)
        FFX_menu.equipWeapon(character=4, ability=0x8002)  # Initiative
        FFX_memory.closeMenu()
    else:
        FFX_menu.addAbility(owner=6, equipment_type=1, ability_array=[
                            0x8072, 255, 255, 255], ability_index=0x800A, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
        FFX_memory.closeMenu()
        ...
        if featherSlot == 255 or FFX_memory.getItemCountSlot(featherSlot) < 6:
            while featherSlot == 255 or FFX_memory.getItemCountSlot(featherSlot) < 6:
                ...
                featherSlot = FFX_memory.getItemSlot(itemNum=54)
        FFX_menu.addAbility(owner=6, equipment_type=0, ability_array=[
                            0x800B, 0x8000, 0x8001, 255], ability_index=0x8002, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
    ...
    if not gameVars.neArmor() in [0, 4, 6]:
        FFX_menu.equipArmor(character=gameVars.neArmor(),
                            ability=99)  # Unequip
    FFX_memory.closeMenu()
...
def oneMpWeapon():  # Break Damage Limit, or One MP cost
    ...
    FFX_memory.closeMenu()
    FFX_menu.addAbility(owner=0, equipment_type=0, ability_array=[
                        0x807A, 255, 255, 255], ability_index=0x800D, slotcount=2, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
    restockDowns()
    FFX_nem_arenaSelect.arenaMenuSelect(4)
...
def kilikaFinalShop():
    ...
    for y in range(weaponBuys):
        if y == 0:  # First one
            FFX_menu.addAbility(owner=0, equipment_type=1, ability_array=[
                                0x8072, 255, 255, 255], ability_index=0x8075, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=False, fullMenuClose=False)
        elif weaponBuys - y == 1:  # Last one
            FFX_menu.addAbility(owner=0, equipment_type=1, ability_array=[
                                0x8072, 255, 255, 255], ability_index=0x8075, slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
        else:
            FFX_menu.addAbility(owner=0, equipment_type=1, ability_array=[
                                0x8072, 255, 255, 255], ability_index=0x8075, slotcount=4, navigateToEquipMenu=False, exitOutOfCurrentWeapon=True, closeMenu=False, fullMenuClose=False)
...
def finalWeapon():
    ...
    FFX_menu.addAbility(owner=0, equipment_type=0, ability_array=[0x800B, 0x8000, 255, 255], ability_index=0x800D,
                        slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=False, closeMenu=False, fullMenuClose=False)
    FFX_menu.addAbility(owner=0, equipment_type=0, ability_array=[0x800B, 0x8000, 0x800D, 255], ability_index=0x8019,
                        slotcount=4, navigateToEquipMenu=False, exitOutOfCurrentWeapon=True, closeMenu=False, fullMenuClose=False)
    ...
    FFX_menu.addAbility(owner=1, equipment_type=1, ability_array=[
                        0x8072, 255, 255, 255], ability_index=0x800A, slotcount=4, navigateToEquipMenu=False, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
    FFX_menu.addAbility(owner=1, equipment_type=1, ability_array=[0x8072, 0x800A, 255, 255], ability_index=0x801D,
                        slotcount=4, navigateToEquipMenu=True, exitOutOfCurrentWeapon=True, closeMenu=True, fullMenuClose=True)
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="besaid", endGoal=capNum, report=True)
    ...
    while not FFX_memory.getMap() in [194, 374]:
        if FFX_memory.userControl():
            if FFX_memory.arenaFarmCheck(zone="kilika", endGoal=capNum, report=False) and checkpoint < 14:
                ...
                FFX_memory.arenaFarmCheck(
                    zone="kilika", endGoal=10, report=True)
                checkpoint += 1
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="kilika", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
...
def mrrFarm(capNum: int = 1):
    ...
    while not FFX_targetPathNem.setMovement([-45, -733]):
        pass
    while not FFX_targetPathNem.setMovement([-61, -692]):  # Past magus sisters
        pass
    # Through Clasko trigger
    while not FFX_targetPathNem.setMovement([-19, -528]):
        pass
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="mrr", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
    ...
    while not FFX_memory.getMap() in [194, 374]:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                ...
                if checkpoint < 25:
                    FFX_memory.arenaFarmCheck(
                        zone="djose1", endGoal=capNum, report=True)
                else:
                    FFX_memory.arenaFarmCheck(
                        zone="djose2", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="tPlains", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
    ...
    while not FFX_memory.getMap() in [194, 374]:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                ...
                if checkpoint < 30:
                    FFX_memory.arenaFarmCheck(
                        zone="bikanel1", endGoal=capNum, report=True)
                else:
                    FFX_memory.arenaFarmCheck(
                        zone="bikanel2", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
    initArray = FFX_memory.checkAbility(ability=0x8002)
    if initArray[4]:
    ...
    while not FFX_memory.getMap() == 307:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                ...
                if checkpoint < 6:
                    FFX_memory.arenaFarmCheck(
                        zone='calm', endGoal=capNum, report=True)
                else:
                    FFX_memory.arenaFarmCheck(
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="gagazet1", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
    ...
    while not checkpoint == 11:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="gagazet2", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                if capNum == 10:
                    ...
                    battleFarmAll()
                FFX_memory.arenaFarmCheck(
                    zone="gagazet3", endGoal=capNum, report=True)
            elif FFX_memory.menuOpen() or FFX_memory.diagSkipPossible():
                FFX_Xbox.tapB()
    ...
    while not FFX_memory.getMap() in [194, 374]:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                ...
                if checkpoint < 35:
                    FFX_memory.arenaFarmCheck(
                        zone="stolenfayth1", endGoal=capNum, report=True)
                elif checkpoint < 43:
                    FFX_memory.arenaFarmCheck(
                        zone="stolenfayth2", endGoal=capNum, report=True)
                else:
                    FFX_memory.arenaFarmCheck(
                        zone="stolenfayth3", endGoal=capNum, report=True)
                FFX_memory.clickToControl()
            elif checkpoint == 43:  # Talking to Fayth
                yojimboDialog()
                checkpoint += 1
        ...
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                ...
                if checkpoint < 40:
                    print("Ahrimans only:")
                    FFX_memory.arenaFarmCheck(
                        zone="sin1", endGoal=capNum, report=True)
                else:
                    FFX_memory.arenaFarmCheck(
                        zone="sin2", endGoal=capNum, report=True)
    ...
    while not FFX_memory.getMap() in [194, 374]:
        if FFX_memory.userControl():
            ...
            if FFX_memory.battleActive():
                advancedBattleLogic()
                FFX_memory.arenaFarmCheck(
                    zone="omega", endGoal=capNum, report=True)
                FFX_memory.clickToControl()
