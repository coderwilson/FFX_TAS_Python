          ...
          #  print("Main menu cursor")
           ...
           # print("Done with menu cursor")
            ...
            while FFX_memory.getCharCursorPos() != target_pos:
                ...
                if FFX_memory.getStoryProgress() == 2528:
...
def performNextGrid(limit: int = 255):
    ...
    if FFX_memory.getTidusSlvl() >= nextAPneeded(gameVars.nemCheckpointAP()):
        ...
        if gameVars.nemCheckpointAP() == 1:
            ...
            print("----------------------------")
            gameVars.setNemCheckpointAP(
                gameVars.nemCheckpointAP() - 1)  # Decrement
        gameVars.setNemCheckpointAP(
            gameVars.nemCheckpointAP() + 1)  # Increment
...
def nemGridding1():
    ...
    # FFX_menuGrid.selSphere('power','none') #HP sphere
...
def nemGridding3():
    ...
    gridLeft()
    FFX_menuGrid.moveAndUse()
    # FFX_menuGrid.selSphere('hp','none')
    # FFX_menuGrid.useAndUseAgain()
    # FFX_menuGrid.selSphere('power','none')
    # FFX_menuGrid.useAndUseAgain()
    # FFX_menuGrid.selSphere('power','none')
...
def nemGridding5():
    ...
    gridRight()
    FFX_menuGrid.moveAndUse()
    FFX_menuGrid.selSphere('power', 'down')
    FFX_menuGrid.useAndQuit()
    FFX_memory.closeMenu()
...
def nemGridding11():  # Back from Ultima to Wakkas grid
    openGrid(0)
    FFX_menuGrid.moveFirst()
    gridLeft()
    gridUp()
...
def nemGridding12():  # Through Kimahri's grid to Rikkus.
    openGrid(0)
    FFX_menuGrid.moveFirst()
    gridLeft()
    gridLeft()
...
# Backtrack to Auto-life, or can go through Yunas grid on refactor.
def nemGridding26():
    openGrid(0)
    FFX_menuGrid.moveFirst()
    gridRight()
...
def rikkuHaste():
    ...
    gridUp()
    FFX_menuGrid.moveAndUse()
    FFX_menuGrid.selSphere('ability', 'aftersk')
    FFX_menuGrid.useAndQuit()
    FFX_memory.closeMenu()
