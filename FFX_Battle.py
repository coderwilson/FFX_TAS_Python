...
def MiihenRoad(selfDestruct=False):
    ...
    while not FFX_memory.battleComplete():  # AKA end of battle screen
        ...
        if FFX_memory.turnReady():
            if FFX_Screen.turnTidus():
                print("Mi'ihen battle. Self-destruct:",
                      gameVars.selfDestructGet())
        ...
        if FFX_memory.turnReady():
            ...
            if turnchar == 0:
                ...
                if kimahriconfused == True:
                    ...
                    # Old logic, based on not using Stone Breath
...
def oblitzRngWait():
    ...
    if gameVars.usePause():
        ...
        goodRngList = [345511282, 41645750, 323111951,
                       1827503098, 562491965, 2040111393, 281000320, 351049783]
        ...
        if FFX_memory.turnReady():
            if FFX_Screen.faintCheck() >= 1:
                ...
                if not advanceComplete:
                    ...
                    elif FFX_memory.getBattleCharSlot(6) >= 3 and FFX_memory.overdriveState()[6] != 100:
                        buddySwapRikku()
                    elif FFX_memory.getBattleCharSlot(3) >= 3:
                        buddySwapKimahri()
                    else:
                        print("+++Registering turn, someone defending")
                        defend()
                else:
    ...
    while FFX_memory.battleActive():
        if FFX_memory.getBattleNum() == 321:
            ...
            if FFX_Screen.turnYuna():
                ...
                if aeonTurn:
                    aeonDismiss()
                elif advances >= 2 and not attackCount:
                    impulse()
                    attackCount = True
