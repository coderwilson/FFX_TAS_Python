                    ...
                    if FFX_Screen.PixelTestTol(1069,28,(49, 143, 164),5) and woodsVars[1] == 0: #Chimera confirmed
                        ...
                        while not FFX_Screen.BattleScreen():
                            if FFX_Screen.PixelTest(884,91,(225, 225, 225)):
                                woodsVars[1] = 1
                    elif FFX_Screen.PixelTestTol(649,10,(32, 85, 109),5) and woodsVars[2] == 0: #One bee, two blue ele's confirmed
                        print("Double blue elemental")
                        StealDown()
                        while not FFX_Screen.BattleScreen():
                            if FFX_Screen.PixelTest(934,94,(219, 219, 219)):
                                woodsVars[2] = 1
                    elif FFX_Screen.PixelTestTol(1132,61,(52, 151, 170),5) and woodsVars[2] == 0: #Blue ele with tanker and raptor confirmed
...
def negator(): #AKA crawler
    ...
    FFX_Screen.clickToBattle()
    useItem(2,'left') #Throw non-lightning marble at negator
    time.sleep(0.5)
    FFX_Screen.awaitTurn()
    if preEmpt == False:
        ...
        FFX_Screen.awaitTurn()
        useItem(1,'left') #Throw lightning marble at negator if it's still up.
        time.sleep(0.5)
...
def fullheal(healerposition: int, targetposition: int, direction: str):
    ...
    if FFX_memory.getItemSlot(2) < 255:
        itemnum = 2
        itemname = "X-Potion"
    elif FFX_memory.getItemSlot(8) < 255:
        itemnum = 8
        itemname = "Elixir"
    elif FFX_memory.getItemSlot(6) < 255:
        itemnum = 6
    ...
    if itemnum >= 0:
        ...
        time.sleep(0.3)
        cursor = 1
        itemPos = FFX_memory.getItemSlot(itemnum)
        if itemPos % 2 == 0:
            FFX_Xbox.menuRight()
            cursor += 1
        if cursor == itemPos:
...
def wendigoresheal(turnchar: int, powerbreak: int, tidusmaxHP: int):
    partyHP = FFX_memory.getBattleHP()
    if FFX_Screen.faintCheck() == 2:
        if FFX_memory.getItemSlot(7) < 255:
            reviveAll()
        elif FFX_memory.getItemSlot(6) < 255:
            revive()  # This should technically target tidus but need to update this logic
    # If just Tidus is dead revive him
    elif powerbreak == 1:
        swapposition = (FFX_memory.getBattleCharSlot(2) - 3) % 4
        buddySwap(swapposition)
        ...
        if FFX_Screen.BattleScreen():
            ...
            if turnchar == 1:
                ...
                if YunaAP == False:
                    ...
                    if wendigoresheal(turnchar=turnchar, powerbreak=powerbreak, tidusmaxHP=tidusmaxHP) == 0:
                        defend()
                    YunaAP = True
                # If Yuna has had a turn swap for Lulu
                    ...
                    if powerbreak == 1:
                        ...
                        swapindex = 5
                    swapposition = (FFX_memory.getBattleCharSlot(swapindex)-3) % 4
                    buddySwap(swapposition)  # Swap for Lulu/Auron
            elif turnchar == 0:
                if tidushaste == False:
                    FFX_Xbox.tidusHaste('none')
                    tidushaste = True
                ...
                if phase == 1:
                    phase += 1
                    lightcurtainslot = FFX_memory.getItemSlot(57)
                    if lightcurtainslot < 255:
                        ...
                        swapposition = (FFX_memory.getBattleCharSlot(2) - 3) % 4
                        buddySwap(swapposition)  # Swap for Auron
                        powerbreak += 1
                elif wendigoresheal(turnchar=turnchar, powerbreak=powerbreak, tidusmaxHP=tidusmaxHP) == 0:
                    if guadosteal == False:
                        Steal()
                        guadosteal = True
                    else:
        ...
        if FFX_Screen.BattleScreen():
            ...
            if turnchar == 0:
                if tidusPrep == 0:
                    ...
                    tidusPrep += 1
                    cheer()
                elif tidusPrep == 2 and otherTurns == 0:
                    tidusPrep += 1
